...
# Summary of Final Experiment and Recommendation
The final experiment showed the strongest overall performance across all experiments, particularly in text generation quality and structural image reconstruction. The Roberta model achieved the best text metrics so far with a BLEU score of 0.1494, ROUGE-L of 0.4091, and METEOR score of 0.4621, while also significantly improving the SSIM score to 0.2305 compared to previous experiments. Although the generated images are still blurry, they now show stronger scene structure, object positioning, and spatial consistency rather than pure noise or abstract blur seen in earlier models.

The single image reconstruction test demonstrated that the upgraded visual autoencoder has enough capacity to reconstruct detailed images when trained on a simplified task. By step 1400, the model reconstructed a single image to approximately 95% accuracy, confirming that the encoder and decoder architecture itself is capable of high-quality reconstruction. This suggests that the current limitation is no longer basic image generation capacity, but rather multimodal temporal prediction and latent alignment inside the sequence predictor.

The upgraded CLIP visual encoder contributed significantly to the improvement in image quality. Unlike previous experiments that only used the global CLIP embedding, the final model extracted multi-scale spatial feature maps from multiple Vision Transformer layers using Patch16 representations. This preserved more spatial information such as edges, textures, object boundaries, and semantic hierarchy. Residual decoding blocks, skip connections, progressive upsampling, and perceptual loss further improved reconstruction stability and perceptual quality.

The generated text also became noticeably more coherent compared to earlier experiments. While the Roberta model still repeats words and loses narrative consistency over long sequences, it now preserves scene structure, emotional tone, and partial semantic relationships between characters and environments. The examples show that the model can generate contextually related words and maintain partial sentence flow, indicating that the RoBERTa encoder and upgraded decoder are learning stronger latent language representations.

The attention heatmaps show a more balanced temporal understanding than earlier experiments, where some attention weights were nearly completely ignored and fell below 0.1. In this run, all four input frames receive very similar average attention, ranging from about 0.248 to 0.251, with input frame 1 slightly highest, followed closely by frame 4, frame 2, and frame 3. This suggests the model is no longer concentrating almost entirely on a single frame and is instead distributing temporal importance more evenly across the sequence.

CLIP’s transformer progressively converts broad spatial patch relationships into concentrated semantic understanding focused on the most informative region of the image.

The cross-modal alignment visualization showed that image and text embeddings were still highly similar across many samples, indicating that the shared latent space remains partially collapsed. Even after rebalancing the multimodal loss using higher image weighting (image ×4) and reduced text weighting (text ×0.7), the text modality continued to dominate the latent space. This likely explains why text generation improved more strongly than image generation and suggests that the model still struggles to fully separate and align visual-text semantic representations.

Overall, the experiments demonstrate that the RoBERTa and CLIP encoders are no longer the main bottleneck. The single-image reconstruction test proved that the visual autoencoder can generate detailed outputs, while the improved text metrics confirmed the strength of the pretrained language representations. The current limiting factor appears to be the sequence predictor and multimodal fusion mechanism, which may not yet be powerful enough to fully utilize the richer latent representations being produced by both encoders.

Future improvements should focus on replacing the LSTM decoder with a transformer-based decoder to better match the transformer architecture of RoBERTa, while the visual generation pipeline would likely benefit from diffusion-style denoising decoders or iterative refinement methods for sharper future-frame prediction. Cross-attention mechanisms between image and text modalities may also improve latent alignment and reduce representation collapse within the shared embedding space.
